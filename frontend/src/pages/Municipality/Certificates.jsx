...
const certTypes = [
  { id: 'birth',    label: 'Birth Certificate',    icon: Baby,        color: 'bg-blue-100 text-blue-600',   fields: ['Child Full Name','Date of Birth','Place of Birth','Father Name','Mother Name'] },
  { id: 'death',    label: 'Death Certificate',    icon: AlertCircle, color: 'bg-slate-100 text-slate-600', fields: ['Deceased Full Name','Date of Death','Place of Death','Applicant Name','Relation'] },
  { id: 'marriage', label: 'Marriage Certificate', icon: Heart,       color: 'bg-pink-100 text-pink-600',   fields: ['Groom Full Name','Bride Full Name','Date of Marriage','Place of Marriage','Witness Name'] },
];
    ...
    <div className="max-w-4xl mx-auto w-full">
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800">Certificate Services</h2>
            <p className="text-slate-500 text-sm">Kaun sa certificate chahiye?</p>
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
      <button onClick={() => setSelected(null)} className="text-blue-600 font-bold mb-4 flex items-center gap-2 hover:underline">
        ← Certificate type change karein
      </button>
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Sahi details bharein</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {selected.fields.map((field, i) => (
              <div key={i}>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">{field}</label>
                <input
                  type={field.includes('Date') ? 'date' : 'text'}
                  placeholder={field.includes('Date') ? '' : `Enter ${field}`}
          ...
          <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
            <p className="text-xs font-bold text-blue-700">📋 Required: ID Proof + Supporting Documents</p>
            <p className="text-xs text-blue-600 mt-1">Application Fee: ₹50 | Processing: 7 working days</p>
          </div>
