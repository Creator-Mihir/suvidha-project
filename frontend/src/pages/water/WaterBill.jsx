...
export default function WaterBill({ connId, setModal }) {
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-24 gap-3 text-slate-400">
      ...
      <span className="text-xl font-semibold">Fetching your bill...</span>
    </div>
  );
  ...
  return (
    <div className="bg-white p-10 rounded-[40px] shadow-lg w-full max-w-6xl">
      <div className="flex justify-between items-start mb-10">
        <div>
          <h2 className="text-3xl font-bold text-slate-800">Water Usage Bill</h2>
          <p className="text-slate-500">Billing Period: <span className="font-semibold text-slate-700">{billData?.billingPeriod}</span></p>
        </div>
        <div className="bg-blue-600 text-white px-6 py-2 rounded-full font-bold">DUE: {billData?.dueDate}</div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100">
            <h4 className="text-xs uppercase font-bold text-slate-400 mb-4 tracking-widest">Consumer Details</h4>
            <div className="grid grid-cols-2 gap-y-6">
              <div><p className="text-sm text-slate-500">Name</p><p className="text-xl font-bold">{billData?.consumerName}</p></div>
              <div><p className="text-sm text-slate-500">ID</p><p className="text-xl font-bold">{connId}</p></div>
              <div className="col-span-2"><p className="text-sm text-slate-500">Address</p><p className="text-lg">{billData?.address}</p></div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="bg-white p-4 rounded-2xl shadow-sm">
                <p className="text-xs text-slate-500">Prev Reading</p>
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border-2 border-blue-100">
                <p className="text-xs text-slate-500">Curr Reading</p>
