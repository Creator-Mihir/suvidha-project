    ...
    <div className="max-w-4xl mx-auto w-full space-y-6">
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800">Property Tax Payment</h2>
            <p className="text-slate-500 text-sm">Select property and pay instantly</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-16 gap-3 text-slate-400">
            <Loader2 className="animate-spin" size={24} /><span>Loading properties...</span>
          </div>
        ) : (
          <>
            {/* Property Select */}
            <div className="mb-6">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-3">Select Property</label>
            ...
            {/* Period Toggle */}
            <div className="mb-6">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-3">Payment Period</label>
            ...
            <div className="bg-gradient-to-r from-purple-700 to-purple-500 p-6 rounded-[1.5rem] text-white mb-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-xs font-bold uppercase opacity-80">Tax Amount</p>
                  <p className="text-4xl font-black mt-1">₹{amount.toLocaleString()}.00</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold uppercase opacity-80">Financial Year</p>
            ...
            <div className="bg-slate-50 p-5 rounded-2xl space-y-3 mb-6 border border-slate-100">
              <div className="flex justify-between text-sm"><span className="text-slate-500">Base Tax</span><span className="font-bold">₹{Math.floor(amount * 0.8).toLocaleString()}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-500">Education Cess (10%)</span><span className="font-bold">₹{Math.floor(amount * 0.1).toLocaleString()}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-500">Swachh Bharat Tax (10%)</span><span className="font-bold">₹{Math.floor(amount * 0.1).toLocaleString()}</span></div>
              <div className="flex justify-between font-black text-base border-t border-slate-200 pt-3"><span>Total</span><span>₹{amount.toLocaleString()}</span></div>
            </div>
