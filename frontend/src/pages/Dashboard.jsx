...
import {
  ...
  FileText, MapPin, ArrowRight, User
} from "lucide-react";
...
function Dashboard() {
  ...
  useEffect(() => {
    // First try localStorage (instant, no API call)
    const user = getUser();
    if (user?.full_name) {
    ...
    // Then fetch fresh from citizen-service
    getProfile()
      .then((res) => {
      ...
      .catch(() => {
        // citizen-service unavailable — just show mobile
      });
  }, []);
